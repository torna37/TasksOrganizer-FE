...
const AllTasksPage: React.FC = () => {
  ...
  const loadTasksAndOccurrences = async () => {
    try {
      setIsLoading(true);
      const allTasks = await TaskApi.getAllTasks(filterAssigned ? 'current-user' : undefined);
      const allOccurrences = await TaskApi.getAllTaskOccurrences(filterAssigned ? 'current-user' : undefined);
      setTasks(allTasks);
  ...
  const overdue = occurrences.filter(o => new Date(o.dueDate) < today && !o.completed);
  const todayTasks = occurrences.filter(o => {
    const dueDate = new Date(o.dueDate);
    dueDate.setHours(0, 0, 0, 0);
    return dueDate.getTime() === today.getTime() && (!o.completed || showAllTasks);
  });
  const tomorrowTasks = occurrences.filter(o => {
    const dueDate = new Date(o.dueDate);
    dueDate.setHours(0, 0, 0, 0);
    return dueDate.getTime() === tomorrow.getTime() && (!o.completed || showAllTasks);
  });
  const thisWeekTasks = occurrences.filter(o => {
    const dueDate = new Date(o.dueDate);
    dueDate.setHours(0, 0, 0, 0);
    return dueDate > tomorrow && dueDate < nextWeek && (!o.completed || showAllTasks);
  });
  ...
  const completedTasks = showAllTasks ? occurrences.filter(o => o.completed) : [];
